...
export function createPlatformProvider(): EmbeddingProvider {
  ...
  return {
    ...
    async initialize(path?: string) {
      ...
      await NativeEmbeddingGemma.load(path, EMBEDDING_DIMENSIONS);
    },
    async embedQuery(text) {
      return NativeEmbeddingGemma.embed(QUERY_PREFIX + text);
    },
    async embedDocuments(texts, titles = []) {
      return Promise.all(
        texts.map((t, i) => NativeEmbeddingGemma.embed(DOCUMENT_PREFIX(titles[i] || 'none') + t)),
      );
    },
  };
}
