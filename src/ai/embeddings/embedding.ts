...
export type EmbeddingStatus = {
  kind: 'missing' | 'downloading' | 'ready' | 'fallback';
  progress: number;
  label: string;
};
export interface EmbeddingProvider {
  ...
  embedDocuments(texts: string[], titles?: string[]): Promise<number[][]>;
}
...
export class HashEmbeddingProvider implements EmbeddingProvider {
  ...
  async embedQuery(text: string) {
    return hash(QUERY_PREFIX + text);
  }
  async embedDocuments(texts: string[], titles: string[] = []) {
    return texts.map((t, i) => hash(DOCUMENT_PREFIX(titles[i] || 'none') + t));
  }
}
