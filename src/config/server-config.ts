...
export const DEFAULT_MODEL_SERVER_URL =
  process.env.EXPO_PUBLIC_MODEL_SERVER_URL || 'http://127.0.0.1:8787';
...
export async function loadServerUrl() {
  ...
  if (saved) {
    try {
      current = normalizeServerUrl(saved);
    } catch {
      current = DEFAULT_MODEL_SERVER_URL;
    }
  }
  return current;
}
